...
setwd("/Users/zhangh24/GoogleDrive/multi_ethnic/result/LD_simulation_new/venn")
load("snp.infor.rdata")
library(VennDiagram)
...
# Chart
venn.diagram(
  x = list(EUR, AFR, AMR,EAS,SAS),
  category.names = c("EUR","AFR ","AMR","EAS","SAS"),
  ...
  lwd = 3,
  lty = 'blank',
  fill = c(alpha("#386cb0",0.5), alpha("#EF7E3D",0.5), alpha("#FFD042",0.5),alpha("#7fc97f",0.5),alpha("#004953",0.5)),
...
#select other SNPs
#15 different combinations total
#the number of causal SNPs are proportion to number of SNPs group
n.cau.EUR <- 5000
...
# Chart
venn.diagram(
  x = list(cau.EUR, cau.AFR, cau.AMR,cau.EAS),
  category.names = c("EUR" , "AFR " , "AMR","EAS"),
  filename = '#causal_venn_diagramm.png',
  output=TRUE,
  ...
  lwd = 3,
  lty = 'blank',
  fill = c(alpha("#386cb0",0.5), alpha("#EF7E3D",0.5), alpha("#FFD042",0.5),alpha("#7fc97f",0.5)),
