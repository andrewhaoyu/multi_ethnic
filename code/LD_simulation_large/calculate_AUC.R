...
r2.mat.test <- rep(0,length(pthres))
r2.mat.vad <- rep(0,length(pthres))
...
setwd("/data/zhangh24/multi_ethnic/")
  ...
  y <- y[,2+(1:n.rep)]
  n <- nrow(y)
  y_test_mat <- y[(n.train[i]+1):nrow(y),]
  ...
  for(k in 1:length(pthres)){
    ...
    if(length(jdx)>0){
      ...
      filename <- paste0(cur.dir,eth[i],"/prs/prs_",k,"_rho_",l,"_size_",m,".profile")
      ...
      r2.mat.test[k] <- mean(r2.test.rep)
      r2.mat.vad[k] <- mean(r2.vad.rep)
    }else{
      r2.mat.test[k] = 0
      r2.mat.vad[k] = 0
    }
