...
for(i in 1:5){
  ...
  dim(sum.data)
  jdx <- which(sum.data$rsid%in%snp.id)
  print(length(jdx))
  
}

#load snp summary information
...
idx <- which(is.na(snpinfo_mega$im.data.id))
snp.id = snpinfo_mega$assay.name[idx]


sum.data.match = inner_join(sum.data,snpinfo_mega_filter,by= c("rsid"="assay.name"))
dim(sum.data.match)
