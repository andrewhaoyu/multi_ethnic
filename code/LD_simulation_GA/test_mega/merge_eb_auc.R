    ...
    for(l in 1:3){
      for(m in 1:4){
        ...
        for(i_rep in 1:n.rep){
          filename = paste0(out.dir,eth[i],"/r2.list_eb_i_c_",i_c,"_rho_",l,"_size_",m,"_rep_",i_rep,"_GA_",i1)
        ...
        method_vec[temp:(temp+1)] <- chip_vec[i_c]
        temp = temp+1
      }
    }
  }
...
EB.result.chip <- data.frame(eth.vec,r2.vec,l_vec,m_vec,method_vec,ga_vec)
save(EB.result.chip,file = paste0(out.dir,"EB.result.chip"))
