...
if(q==1){
  ...
  hm.list <- as.data.frame(fread(paste0(cur.dir,"hm3rsid.txt")))
  colnames(hm.list)[1] = "rs_id"
  hm.list = hm.list[,1,drop=F]
  sum.data.match <- inner_join(hm.list,sum.data,by="rs_id")
}
