      ...
      for(k2 in 1:length(pthres)){
        prs.all <- prs.clump %>%
          filter(peur<=pthres[k1]|
                   P<=pthres[k2])
        if(nrow(prs.all)>0){
          filename <- paste0(out.dir,eth[i],"/prs/prs_alleth_eb_rho_",l,"_size_",m,"_rep_",i_rep,"_GA_",i1,"_rind_",r_ind,"_wcind_",w_ind,"p_value_",k1,".p_value_",k2,".profile")
...
for(r_ind in 1:length(r2_vec)){
  ...
  for(w_ind in 1:length(wc_vec)){
    for(eth_ind in 2:2){
      prs.mat.temp <- fread(paste0(out.dir,eth[i],"/prs/prs_alleth_eb_rho_",l,"_size_",m,"_rep_",i_rep,"_GA_",i1,"_rind_",r_ind,"_wcind_",w_ind,"_eth_ind_",eth_ind,".profile"))    
      prs.list[[temp]] = prs.mat.temp[,3:ncol(prs.mat.temp)]
      temp = temp + 1
    }
    
  }
}
#bind the prs based on the other ethnicities post prs
prs.other.mat = bind_cols(prs.list)

#combine the prs matrix
prs.all.mat = cbind(prs.mat,prs.other.mat)
prs.all.mat = as.data.frame(prs.all.mat)
prs.sum = colSums(prs.all.mat)
idx <- which(prs.sum!=0)
#drop the prs with all 0
prs.all.mat <- prs.all.mat[,idx]
#drop the columns with perfect correlation

mtx = cor(prs.all.mat[1:n.test,])
library(caret)
drop = findCorrelation(mtx,cutoff=0.98)
drop = names(prs.all.mat)[drop]
prs.mat.new = prs.all.mat %>% 
  select(-all_of(drop))
