...
R2.ldpred2 = R2.ldpred2 %>% 
  rename(eth.vec = race,
         ...
         ga_vec = GA,
         method_vec = Method)
load(paste0("R2.weightedLDpred2.sim.RData"))
R2.weightedLDpred2 = R2.weightedLDpred2 %>%
  ...
  rename(eth.vec = race,
         ...
         ga_vec = GA,
         method_vec = Method)
load("mebayes-sim.rdata")
R2.mebayes.sl = R2.mebayes.sl %>% 
  mutate(Method = "MEBayes") %>% 
  rename(eth.vec = race,
         r2.vec = R2,
         l_vec = rho,
         m_vec = size,
...
lasso = fread("simulation_jingning.txt") %>% 
  filter(method_vec =="MElasso") %>% 
  mutate(method_vec = "MELasso")
...
setwd("/Users/zhangh24/GoogleDrive/multi_ethnic/result/LD_simulation_GA/LD_stack/simulation_figure_all_combine")
library(cowplot)
m = 1
for(m in 1:4){
  for(i1 in 1:5){
