...
EB.result.chip = EB.result.chip %>% 
  filter(method_vec=="HM3") %>% 
  mutate(method_vec="CT-SLEB")
prediction.result <- rbind(prscsx.result,
                           EB.result.chip)
...
for(m in 1:4){
  for(i1 in 1:1){
    ...
    #print(p)
    #print(p)
    png(file = paste0("./HM3_CTSLEB_PRSCSx/HM3_compare_",m,"_summary_GA_",i1,".png"),
