...
load(paste0("CTSLEB_test_correlation.rdata"))
...
prediction.result = prediction.result %>% 
  ...
  mutate(cau_vec = factor(cau_vec,
         ...
         sample_size = factor(sample_size,
                              levels = c("15000")),
         cor_val = factor(cor_val,
                             levels = c("0.50", "0.70", "0.90", "0.98"))) %>% 
...
setwd("/Users/zhangh24/GoogleDrive/multi_ethnic/result/LD_simulation_GA/LD_stack/simulation_correlation_cutoff/")
library(cowplot)
m = 1
...
#print(p.leg)
p <- ggplot(prediction.result.sub,aes(x= sample_size,y=r2.vec,group=cor_val))+
  geom_bar(aes(fill=cor_val),
           stat="identity",
...
png(file = paste0("./correlation_cutoff.png"),
