...
for(i in 2:5){
  ...
  for(i1 in 1:1){
    ...
    for(l in 1:3){
      for(m in 1:4){
        r2.temp <- rep(0,n_rep)
        nprs_vec.temp = rep(0,n_rep)
        
        
        
        for(i_rep in 1:n.rep){
          filename = paste0(out.dir,eth[i],"/num_prs_rho_ebtest_",l,"_size_",m,"_rep_",i_rep,"_GA_",i1)
          
          load(filename)
          nprs_vec.temp[i_rep] = num_prs
        ...
        nprs_vec[temp] <- mean(nprs_vec.temp)
        l_vec[temp] <- rep(l,1)
        m_vec[temp] <- rep(m,1)
        ga_vec[temp] <- i1
...
nprs_result <- data.frame(eth.vec,nprs_vec,l_vec,m_vec,method_vec,ga_vec)
library(dplyr)
gr.result %>% group_by(eth.vec) %>% 
  summarize(average = mean(gr_vec))
