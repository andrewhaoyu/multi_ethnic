...
sum_eur <- as.data.frame(fread(paste0(out.dir.sum,eth[1],"/summary_mega_rho_",l,"_size_",4,"_rep_",i_rep,"_GA_",i1)))  
sum_eur = sum_eur %>% 
  mutate(A1 = effect_allele,
         SE = BETA/Z) %>% 
  select(SNP, CHR, BP, BETA, SE, A1, P)
#load target population data
sum_tar = as.data.frame(fread(paste0(out.dir.sum,eth[i],"/summary_mega_rho_",l,"_size_",m,"_rep_",i_rep,"_GA_",i1)))  
...
for(r_ind in 1:length(r2_vec)){
  ...
  for(w_ind in 1:length(wc_vec)){
    ...
    system(paste0(soft.dir,"plink2 ",
                  "--bfile ",temp.dir,eth[1],"clump_ref_all_chr ",
    ...
    system(paste0(soft.dir,"plink2 ",
                  "--bfile ",temp.dir,eth[i],"clump_ref_all_chr ",
                  "--clump ",temp.dir,"sum_tar_ref ",
                  "--clump-p1 ",pthr," ",
                  "--clump-r2 ",r2thr," ",
                  "--clump-kb ",kbpthr," ",
                  "--out ", temp.dir,eth[i],"_ref_CT_rind_",r_ind,"_wcind_",w_ind))
    ...
    LD_tar = fread(paste0(temp.dir,eth[i],"_ref_CT_rind_",r_ind,"_wcind_",w_ind,".clumped"))[,3,drop=F]
...
for(i_eth in 1:length(eth_other)){
  ...
  sum_temp = as.data.frame(fread(paste0(data.dir,eth_vec[i_eth],"/",trait,".txt"),header=T))
