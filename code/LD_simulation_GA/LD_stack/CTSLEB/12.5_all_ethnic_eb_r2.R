      ...
      for(k2 in 1:length(pthres)){
        prs.all <- prs.clump %>%
          filter(peur<=pthres[k1]|
                   P<=pthres[k2])
        if(nrow(prs.all)>0){
          filename <- paste0(out.dir,eth[i],"/prs/prs_alleth_eb_rho_",l,"_size_",m,"_rep_",i_rep,"_GA_",i1,"_rind_",r_ind,"_wcind_",w_ind,"p_value_",k1,".p_value_",k2,".sscore")
...
save(r2.list,file = paste0(out.dir,eth[i],"/r2.list_alleth_update_rho_eb_",l,"_size_",m,"_rep_",i_rep,"_GA_",i1))
