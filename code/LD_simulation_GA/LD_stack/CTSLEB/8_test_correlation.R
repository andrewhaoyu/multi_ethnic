...
n <- nrow(y)
y_test_mat <- y[(100000+1):(100000+n.test+n.vad),,drop=F]
...
prs.mat <- matrix(0,20000,length(pthres)^2*length(r2_vec)*length(wc_base_vec))
temp = 1
...
library(caret)
cor_cutoff = cor_vec[k]
drop = findCorrelation(mtx,cutoff=cor_cutoff)
...
#r2_result[k] = r2.stack

#}
save(r2.stack,file = paste0(out.dir,eth[i],"/r2_cor_cut_",k,"_rho_",l,"_size_",m,"_rep_",i_rep,"_GA_",i1))
