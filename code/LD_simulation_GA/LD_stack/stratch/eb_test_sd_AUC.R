      ...
      for(k2 in 1:length(pthres)){
        prs.all <- prs.clump %>%
          filter(peur<=pthres[k1]|
                   P<=pthres[k2])
        if(nrow(prs.all)>0){
          filename <- paste0(out.dir,eth[i],"/prs/prs_eb_test_sd_rho_",l,"_size_",m,"_rep_",i_rep,"_GA_",i1,"_rind_",r_ind,"_wcind_",w_ind,"p_value_",k1,".p_value_",k2,".profile")
...
r2.list <- list(r2.stack,
                r2.max,
                result.data[idx,])
save(r2.list,file = paste0(out.dir,eth[i],"/r2.list_rho_eb_test_sd_",l,"_size_",m,"_rep_",i_rep,"_GA_",i1))
