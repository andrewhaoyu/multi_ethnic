...
for(v in 1:4){
  ...
  hm3.snp.infor = as.data.frame(fread("/data/zhangh24/software/PRScsx/1KGLD/snpinfo_mult_1kg_hm3"))
  hm3.snp.infor = hm3.snp.infor %>% select(SNP)
  
  prs_infor = inner_join(prs, hm3.snp.infor, by = c("V2"="SNP")) %>% 
    select(V2,V4)
  colnames(prs_infor) = c("rs_id","A1")
  n_rep = 10
  Beta = matrix(0,nrow(prs_infor),n_rep)
  for(i_rep in 1:n_rep){
    print(i_rep)
    prs = fread(paste0("rho_",l,"_size_",m,"_rep_",i_rep,"_GA_",i1,"_",eth[i],
                       "_pst_eff_a1_b0.5_phi",phi[v],".txt"))
    
    prs = left_join(prs_infor, prs, by = c("rs_id"= "V2"))
  ...
  system(paste0("mv ",temp.dir.prs,"/prs_csx_",eth[i],"_rho_",l,"_size_",m,"_GA_",i1,"_phi",phi[v],".sscore ",out.dir.sum,eth[i],"/prscsx_megahm3/"))
  ...
  system(paste0("mv ",temp.dir.prs,"/prs_csx_EUR_rho_",l,"_size_",m,"_GA_",i1,"_phi",phi[v],".sscore ",out.dir.sum,eth[i],"/prscsx_megahm3/"))
