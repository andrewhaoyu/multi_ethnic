...
ct_result = ct_result %>% 
  select(eth_vec, r2, r2_low, r2_high, l_vec, ga_vec, m_vec,method_vec)
...
eur_result = result.table %>% 
  ...
  select(eth_vec,r2.vec,ci_low,ci_high,l_vec,ga_vec,m_vec,method_vec) %>% 
  rename(r2_low = ci_low,
         r2_high = ci_high,
         r2 =r2.vec)
  load(paste0(out.dir,"weightedprs.CT.95CI.rdata"))
  weighted_prs = result.table %>% 
    mutate(method_vec = "Weighted PRS (CT, five ancestries)" ) %>% 
    select(eth_vec,r2,r2_low,r2_high,l_vec,ga_vec,m_vec,method_vec)
...
idx = which(duplicated(final_result))
