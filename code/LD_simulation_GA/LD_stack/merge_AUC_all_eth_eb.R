...
for(i in 2:5){
  ...
  files <- dir(path = filedir,pattern=paste0("r2.list_rho_alletheb_*"),full.names = T)
  for(i1 in 1:1){
    ...
    for(l in 1:3){
      for(m in 1:1){
        ...
        for(i_rep in 1:n.rep){
          filename = paste0(out.dir,eth[i],"/r2.list_rho_alletheb_",l,"_size_",m,"_rep_",i_rep,"_GA_",i1)
        ...
        method_vec[temp:(temp+1)] <- c("TDLD-EB (all ethnics)","TDLD-SLEB (all ethnics)")
        temp = temp+2
      }
    }
  }
...
alleth.EB.result <- data.frame(eth.vec,r2.vec,l_vec,m_vec,method_vec,ga_vec)
save(alleth.EB.result,file = paste0(out.dir,"LD.clump.result.alleth.EB.rdata"))
