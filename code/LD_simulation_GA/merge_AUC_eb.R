...
for(i1 in 1:2){
  ...
  for(i in 2:5){
    all.files <- dir(path=paste0(out.dir,eth[i]),
                     pattern="r2.list_eb_rho_",
                     full.names = T)
    ...
    for(l in 1:3){
      for(m in 1:4){
        ...
        for(i_rep in 1:n.rep){
          filename = paste0(out.dir,eth[i],"/r2.list_eb_rho_",l,"_size_",m,"_rep_",i_rep,"_GA_",i1)
          if(filename%in%all.files){
            load(filename)  
  ...
  save(LD.result.list,file = paste0(out.dir,"LD.clump.result.eb_GA_",i1,".rdata"))
