...
for(i1 in 1:2){
  ...
  for(q in 1:3){
    for(i in 2:5){
      for(l in 1:3){
        for(m in 1:4){
          load(paste0(cur.dir,eth[i],"/r2_eursnp_",method[q],"_rho_",l,"_size_",m,"_GA_",i1))
          eth.vec[temp] = eth[i]
  ...
  colnames(LD.clump.result)[2] <- "r2.vec"
  save(LD.clump.result,file = paste0(cur.dir,"best_eur_snp_result_GA_",i1,".rdata"))
