...
for(i in 2:length(eth)){
  ...
  load(paste0("/data/zhangh24/multi_ethnic/result/LD_simulation/",eth[i],"/LD_clump_eurp_eurref_all_infor.clumped"))
    ...
    for(l in 1:length(pthres)){
      ...
      if(length(kdx)>0){
        ...
        for(j in c(1:22)){
          ...
          filename <- paste0("chr",j,"_eb_eurp_eurref",k,"_",l,".profile")
