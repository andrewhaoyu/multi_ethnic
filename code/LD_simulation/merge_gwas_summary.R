...
code <- c("c",4)
for(i in 1:4){
  temp.code <- paste0("head -1 /data/zhangh24/multi_ethnic/result/LD_simulation/",eth[i],"/summary_chr1.out.pheno1.glm.linear > /data/zhangh24/multi_ethnic/result/LD_simulation/",eth[i],"/summary.out; awk 'FNR>1'")
  for(j in 1:22){
    temp.code <- paste0(temp.code, " /data/zhangh24/multi_ethnic/result/LD_simulation/",eth[i],"/summary_chr",j,".out.pheno1.glm.linear")
  }
