# ------- Load the cross-ancestry PRS and train & validation the weighted LDpred2
rm(list=ls())
library(readr)
library(data.table)
library(mvtnorm)
...
for (trait in traits){
  for (race in races[c(1:K)]){
    ...
    for (j in 1:K){ # training ancestry
      ...
      temprsfile = paste0('/dcs04/nilanjan/data/jjin/prs/realdata/allofus/',trait,'/',race,'/ldpred2/crosspop/',race0,'-',race,'.txt')
      ...
      for(i in 1:nrow(sets)){
        ...
        fit = lm(formula.prs, data=prstable)
        output[i,'R2'] = summary(fit)$r.squared 
      }
    ...
    for (k in 1:K){
      race0 = races[k]
      temfile = paste0('/dcs04/nilanjan/data/jjin/prs/realdata/allofus/',trait,'/',race,'/ldpred2/crosspop/',race0,'-',race,'.txt')
...
save(R2,file=paste0("/dcs04/nilanjan/data/jjin/prs/realdata/allofus/r2/r2-wprs.RData"))
