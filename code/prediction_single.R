...
#LDP select SNPs based on EUR
#regression coefficients use target
LDPsingle <- function(y_all,
                   beta_all,
                   p.train,
                   p.thr,
                   pop.ind){
  ...
  beta.train <- beta_result[[1]][,1]
  beta.train2 <- beta_result[[1]][,4]
  beta.train3 <- beta_result[[1]][,7]
  n.snp <- 0
  for(i in 1:500){
    print(i)
    ...
    for(j in 1:file.snp){
      ...
      if(length(jdx)!=0){
        ...
        prs.temp2 <- genotype[[2]][,j]*beta.train2[n.snp+j] 
        prs.mat2[,jdx] <- prs.mat2[,jdx]+
          prs.temp2[n.train2+(1:(n.test2+n.vad2))]
        prs.temp3 <- genotype[[3]][,j]*beta.train3[n.snp+j] 
  ...
  idx <- which.max(r2.test)
  prs.mat.list <- list(prs.mat2,prs.mat3)
  #record result for AFR and LAC
  result <- rep(0,2)
  for(l in 1:2){
    y = y_all[[l+1]]
    n.sub <- length(y)
    ...
    y.test <- y[n.train+(1:n.test)]
    y.vad <- y[(n.train+n.test)+(1:n.vad)]
    prs.mat <- prs.mat.list[[l]]
    prs.vad <- prs.mat[n.test+(1:n.vad),idx]
    model2 <- lm(y.vad~prs.vad)
    result[l] <- summary(model2)$adj.r.squared
...
#if(pop.ind==1){
  LDP.result <-  LDPsingle(y_all,beta.train,p.train,p.thr,pop.ind)
#}else{
