...
r2_vec_tun = rep(0, 4)
coef_mat = matrix(0, 5, length(eth))

for(v in 1:4){
  score1 = prs_tun[,5*v-4]
  score2 = prs_tun[,5*v-3]
  score3 = prs_tun[,5*v-2]
  score4 = prs_tun[,5*v-1]
  score5 = prs_tun[,5*v]
  model = lm(y_tun~score1+score2+score3+score4+score5)
...
score1 = prs_vad[, 5*max_ind-4]
score2 = prs_vad[, 5*max_ind-3]
score3 = prs_vad[, 5*max_ind-2]
score4 = prs_vad[, 5*max_ind-1]
score5 = prs_vad[, 5*max_ind]
coef = coef_mat[max_ind,]

prs_vad = cbind(score1, score2, score3, 
                score4, score5)%*%coef

model = lm(y_vad~ prs_vad)
r2_true = summary(model)$r.square
...
#take out first two columns since it's SNP and A1
# Select the columns based on max_ind
prs_file = as.data.frame(prs_file)
selected_columns <- seq(from = 5*max_ind-4+2, to = 5*max_ind+2)
final_weight = as.matrix(prs_file[,selected_columns])%*%coef

#verify the results
prs_coef = cbind(prs_file[,1:2],final_weight)
write.table(prs_coef,file = paste0(temp.dir,"prs_prep"),col.names = T,row.names = F,quote=F)
...
for(i_eth in 1:5){
  load(paste0(out.dir.prs,"sum_",eth[i_eth],"_pst_eff_a1_b0.5_phi",phi[v],".rdata"))
  data_temp = data %>% select(V1,V2,V3,V4,V5)
  data_list[[i_eth]] = data_temp
}
snp_set = rbindlist(data_list) %>% distinct()
colnames(snp_set) = c("chr_name", "rsID",
                      "chr_position",
                      "effect_allele",
                      "other_allele")
prs_select = data.frame(snp_set,final_weight=final_weight) %>% 
  rename(effect_weight = final_weight) %>% 
