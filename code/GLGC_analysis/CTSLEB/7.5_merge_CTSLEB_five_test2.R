...
for(l in 1:4){
  for(i in 2:5){
    ...
    file_dir = out.dir
    files = dir(file_dir, pattern  = "CTSLEB_all_test2_",full.names = T)
    for(z_ind in 1:length(pthres)){
      eth = eth_vec[i]
      trait = trait_vec[l]
      file_name = paste0(out.dir, "/CTSLEB_all_test2_",z_ind,".result")
...
save(final_result, file = "/data/zhangh24/multi_ethnic/result/GLGC/CTSLEB/ct_sleb_five_cut_test2.rdata")
