...
#Kevin, I justed named sum_eur as sum_other_ref here.
...
SNP_set = unique_infor[,"SNP"]

##########five ancestries analyses###############
#########EB step###################
EstimatePriorSingle <- function(SNP_set,
                               sum_com){
...
EBpostMultiSingle = function (unique_infor, SNP_set, sum_com){
  prior_sigma = EstimatePriorSingle(SNP_set, 
                                   sum_com)
  SNP_set_select = unique_infor %>% select(SNP)
  SNP_set_align = left_join(SNP_set_select, sum_com, by = "SNP")
  beta_mat = SNP_set_align %>% select(BETA)
  se_mat = SNP_set_align %>% select(SE)
  ...
  colnames(beta_mat_post) = c("BETA_EB_target")
...
unique_infor_post = EBpostMultiSingle(unique_infor,SNP_set,
                                   sum_com)
...
res = system(paste0(soft.dir,"plink2_alpha ",
                    ...
                    "--out ",temp.dir,"eb_prs"))
#the output of plink2 create 9 different files named as eb_prs_k.sscore
...
#column 5-16 are the PRS scores with SNP of p_target<p_thres for different combinations of r2-cutoff and base_window_size
#}
...
for(k2 in 1:length(pthres)){
  ...
  #load PRS for SNPs with p_target<p_thres[k2]
