...
for(i_eth in 1:length(eth_vec)){
  load(paste0("/data/zhangh24/multi_ethnic/result/GLGC/clumping_result/PT/",eth_vec[i_eth],"/",trait,"/CT.result"))
  #find best index
  ...
  sum.data = as.data.frame(fread(paste0(data.dir,eth_vec[i_eth],"/",trait,".txt"),header=T))
  ...
  pthres <- c(5E-08,5E-07,5E-06,5E-05,5E-04,5E-03,5E-02,5E-01,1.0)
  LD <- as.data.frame(fread(paste0("/data/zhangh24/multi_ethnic/result/GLGC/clumping_result/PT/",
                                   eth_vec[i_eth],
...
prs_score = matrix(0, nrow(prs_temp), 5)
for(i_eth in 1:5){
...
colnames(prs_mat)[5:9] = c(paste0("prs_", eth_vec))
...
coeff = coefficients(model.prs)[2:6]
...
#save final prs
out.dir.prs = paste0("/data/zhangh24/multi_ethnic/result/GLGC/prs/weightedprsall/",eth,"/",trait,"/")
...
out_dir_boot = paste0("/data/zhangh24/multi_ethnic/result/GLGC/boot_result/weighted_prs/",eth,"/",trait,"/")
