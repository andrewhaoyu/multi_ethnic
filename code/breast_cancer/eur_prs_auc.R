...
prs.tar.all$BETA = prs.all$Beta.eur
prs.model.file = prs.tar.all %>% 
  mutate(Effect_Allele = ALT) %>% 
  select(KG.ID,Effect_Allele,BETA)

write.table(prs.model.file,
            file = "/data/zhangh24/multi_ethnic/result/breast_cancer/best_eur_eurcoef_prsmodelfile",
            row.names = F,
            col.names = F,
            quote=F,
            sep = "\t")
...
res = auc(prs.model.file = "/data/zhangh24/multi_ethnic/result/breast_cancer/best_eur_eurcoef_prsmodelfile", 
