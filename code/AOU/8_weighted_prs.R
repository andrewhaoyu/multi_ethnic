...
load(paste0("/data/zhangh24/multi_ethnic/result/AOU/clumping_result/PT/",eth,"/",trait,"/CT.result"))
#find best index
idx <- which.max(ct.result[[2]])
#########prepare summary statistics#################
#load eur summary statistics
target = as.data.frame(fread(paste0(data.dir,eth,"/",trait,"_update.txt"),header=T))
sum.data.assoc = sum.data %>% 
  mutate(P = as.numeric(P)) %>% 
  rename(SNP = rsID,
         BP = pos37) %>% 
...
pthres <- c(5E-08,5E-07,5E-06,5E-05,5E-04,5E-03,5E-02,5E-01,1.0)
LD <- as.data.frame(fread(paste0("/data/zhangh24/multi_ethnic/result/AOU/clumping_result/PT/",eth,"/",trait,"/LD_clump.clumped")))
clump.snp <- LD[,3,drop=F]
#prepare the prs files with best EUR SNP and coefficients
prs.all <- left_join(clump.snp,sum.data.assoc) %>% 
  filter(P <= pthres[idx])
