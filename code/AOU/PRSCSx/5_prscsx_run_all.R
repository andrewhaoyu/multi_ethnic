...
sum.eur = as.data.frame(fread(paste0(data.dir,"EUR/",trait,"_update.txt"),header=T))
...
sum.tar = as.data.frame(fread(paste0(data.dir,eth[i],"/",trait,"_update.txt"),header=T))
sum.tar.select = sum.tar %>% 
  rename(SNP=rsID) %>% 
  select(SNP,A1,A2,BETA,P) 
...
system(paste0("python /data/zhangh24/software/PRScsx/PRScsx.py", 
              ...
              " --sst_file=",path_to_sum,"EUR_sumstats.txt,",path_to_sum,"AFR_sumstats.txt,",path_to_sum,"AMR_sumstats.txt,",path_to_sum,
